...
@property (nonatomic, strong) UIButton *secondActionButton;
@property (nonatomic, strong) UIButton *thirdActionButton;
                               ...
                               andParent:(SWBufferedToast *)parentView
{
    ...
    if (self) {
        ...
        [self.secondActionButton setTitle:differentActionTitle forState:UIControlStateNormal];
        [self.thirdActionButton setTitle:thirdActionTitle forState:UIControlStateNormal];
                                  ...
                                  andParent:(UIView*)parentView
{
    ...
    if (self) {
        self.toastType = SWBufferedToastTypeRegister;
        self.parentView = parentView;
        self.registerToastDelegate = registerDelegate;
        ...
        self.usernameField.placeholder = usernameTitle;
        self.passwordField.placeholder = passwordTitle;
        self.firstNameField.placeholder = firstNameTitle;
        [self.actionButton setTitle:doneTitle forState:UIControlStateNormal];
        [self.secondActionButton setTitle:differentActionTitle forState:UIControlStateNormal];
...
- (BOOL)loadingBlocksDismiss
{
    switch (self.toastType) {
        case SWBufferedToastTypeLogin:
            return YES;
            break;
        case SWBufferedToastTypeRegister:
            return YES;
            break;
        default:
            return NO;
...
- (UITextField *)firstNameField
{
    if (!_firstNameField) {
        ...
        _firstNameField.font = [UIFont fontWithName:@"HelveticaNeue-Thin" size:18.0f];
        [_firstNameField setTintColor:[UIColor whiteColor]];
...
{
    if (!_secondActionButton) {
        _secondActionButton = [[UIButton alloc] init];
        [_secondActionButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_secondActionButton setTitleColor:[self darkerColorForColor:self.backgroundColor] forState:UIControlStateDisabled];
        _secondActionButton.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:18.0f];
        [_secondActionButton addTarget:self action:@selector(didTapSecondActionButton:) forControlEvents:UIControlEventTouchUpInside];
    }
    
    return _secondActionButton;
}

- (UIButton *)thirdActionButton
{
    if (!_thirdActionButton) {
        _thirdActionButton = [[UIButton alloc] init];
        [_thirdActionButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_thirdActionButton setTitleColor:[self darkerColorForColor:self.backgroundColor] forState:UIControlStateDisabled];
        _thirdActionButton.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:18.0f];
        [_thirdActionButton addTarget:self action:@selector(didTapThirdActionButton:) forControlEvents:UIControlEventTouchUpInside];
    }
    
    return _thirdActionButton;
}
...
- (void)showBuffer
{
    ...
    if (self.toastType == SWBufferedToastTypeLogin) {
        ...
        [self.secondActionButton setEnabled:NO];
        [self.thirdActionButton setEnabled:NO];
    }
    if (self.toastType == SWBufferedToastTypeRegister) {
        [self.usernameField setEnabled:NO];
        [self.passwordField setEnabled:NO];
...
- (void)hideBuffer
{
    ...
    if (self.toastType == SWBufferedToastTypeLogin) {
        ...
        [self.secondActionButton setEnabled:YES];
        [self.thirdActionButton setEnabled:YES];
    }
    if (self.toastType == SWBufferedToastTypeRegister) {
        [self.usernameField setEnabled:YES];
        [self.passwordField setEnabled:YES];
...
- (void)didTapActionButton:(id)sender
{
    ...
    else if (self.toastType == SWBufferedToastTypeRegister){
        [self performCreateAction];
    }
}
- (void)didTapSecondActionButton:(id)sender
{
    if (self.toastType == SWBufferedToastTypeLogin) {
        [self performRegisterAction];
    } else if (self.toastType == SWBufferedToastTypeRegister)
    {
...
- (void)didTapThirdActionButton:(id)sender
{
...
- (void)performCreateAction
{
    [self.registerToastDelegate registerButtonTappedWithUsername:self.usernameField.text Password:self.passwordField.text andFirstName:self.firstNameField.text];
    [self updateLoginConstraintsToCenter];
    [self endEditing:YES];
}
...
{
    [self.loginToastDelegate registerButtonTapped];
}

- (void)performBackAction
{
    [self.registerToastDelegate backButtonTapped];
}

- (void)performContinueAction
{
    [self.loginToastDelegate continueButtonTapped];
}
...
- (void)applyAllConstraints
{
    ...
    if (self.toastType == SWBufferedToastTypePlain) {
        ...
        [SWToastConstraintManager applyButtonConstraintsForButton:self.actionButton toPasswordField:self.passwordField onToast:self];
        [SWToastConstraintManager applyButtonConstraintsForSecondButton:self.secondActionButton ToFirstButton:self.actionButton onToast:self];
        [SWToastConstraintManager applyButtonConstraintsForButton:self.thirdActionButton onToast:self];
    }
    else if (self.toastType == SWBufferedToastTypeRegister){
        [SWToastConstraintManager applyTextfieldConstraintsForUsernameField:self.usernameField toTitleLabel:self.titleLabel onToast:self];
        [SWToastConstraintManager applyTextfieldConstraintsForPasswordField:self.passwordField toUsernameField:self.usernameField onToast:self];
        [SWToastConstraintManager applyTextfieldConstraintsForFirstNameField:self.firstNameField toPasswordField:self.passwordField onToast:self];
        [SWToastConstraintManager applyButtonConstraintsForButton:self.actionButton toPasswordField:self.firstNameField onToast:self];
        [SWToastConstraintManager applyButtonConstraintsForButton:self.secondActionButton onToast:self];
    }
    else{
...
- (void)updateLoginConstraintsForFirstName
{
    [self.parentView layoutIfNeeded];
    self.constraintY.constant = -115;
    [UIView animateWithDuration:kAnimationDurationAppear
                          delay:0
         usingSpringWithDamping:0.7f
          initialSpringVelocity:15.0f
...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
    if (textField == self.usernameField) {
        ...
        [self updateLoginConstraintsForFirstName];
    }
}
...
- (void)dealloc
{
    ...
    self.registerToastDelegate = nil;
}
